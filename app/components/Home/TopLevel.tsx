      ...
      <div className="w-56 shrink-0 hidden lg:block">
        <ul className="space-y-4">
          ...
          {listItems.map((item, index) => (
            <li key={index} className="flex justify-between items-center cursor-pointer hover:underline">
              <span>{item}</span>
            </li>
          ))}
        </ul>
      ...
      <div className="flex-1 relative">
        <div className="bg-black text-white rounded-sm overflow-hidden flex flex-col md:flex-row items-center justify-between px-8 md:px-16 py-8 md:py-12 min-h-85">
          ...
          <div className="flex flex-col gap-5 z-10 text-center md:text-left">
            ...
            <button className="flex items-center gap-2 text-white hover:opacity-80 underline underline-offset-8 justify-center md:justify-start">
              <span>Shop Now</span>
          ...
          <div className="relative z-10 mt-6 md:mt-0">
            <img src="/app/images/phone.png" alt="iPhone 14 Series" className="w-48 md:w-80 h-auto" />
          </div>
        </div>
        {/* Carousel Indicators */}
        <div className="flex justify-center gap-3 mt-4">
          {
            [...Array(5)].map((_, index) => (
              <div key={index} className={`w-3 h-3 rounded-full ${index === 2 ? 'bg-red-500' : 'bg-gray-300'}`}></div>
            ))
          }
